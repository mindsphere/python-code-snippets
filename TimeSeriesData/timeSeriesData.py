...
def ToDateObject(Date_In : str) -> datetime.datetime:
  ...
  # Main function:
  def timeSeriesData(start_date: str, end_date: str) -> pd.DataFrame:
    ServiceUrl = gateway + IotTimeseriesAPI + 'timeseries/' + deviceId + '/' + propertySetName + '?from=' + start_date + '&to=' + end_date
    ...
    if response.status_code == 200:
        ...
        raise ValueError
    timeseries_data = pd.DataFrame.from_dict(response.json())
